  ...
  create(
    ...
    return {
      ...
      PropertyDefinition(node: TSESTree.PropertyDefinition): void {
        ...
        const isAnArray = typedTokenHelpers.isTypeArrayTypeOrUnionOfArrayTypes(
          node,
          parserServices,
          typeChecker,
        );
        ...
        if (isAnArray) {
          const mainTypeInShortArray = (
            node.typeAnnotation?.typeAnnotation as TSESTree.TSArrayType
          )?.elementType?.type;
        ...
        const hasEnum = foundClassValidatorDecorators.some(
          (foundClassValidatorDecorator) =>
            typedTokenHelpers.decoratorIsIsEnum(foundClassValidatorDecorator),
        );
